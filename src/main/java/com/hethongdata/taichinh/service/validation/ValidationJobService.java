...
public class ValidationJobService {
    ...
    private Outcome ohlc(JsonNode payload) {
        ...
        for (JsonNode object : objects(payload)) {
            ...
            if (low.compareTo(high) > 0)
                return fail(
                        "low=" + low + ", high=" + high, "low <= high", "OHLC bounds are invalid");
            for (BigDecimal value :
                ...
                if (value != null && (value.compareTo(low) < 0 || value.compareTo(high) > 0))
                    return fail(
                            ...
                            "OHLC price is outside its range");
        }
        return candidate
                ? new Outcome("PASS", null, null, "OHLC bounds are valid")
    ...
    private Outcome nonNegativeVolume(JsonNode payload) {
        for (JsonNode object : objects(payload))
            for (String key :
                    List.of("volume", "volume_accumulated", "KhoiLuongKhopLenh", "KLThoaThuan")) {
                BigDecimal number = decimal(object.get(key));
                if (number != null && number.signum() < 0)
    ...
    private Outcome required(JsonNode payload) {
        return payload == null || payload.isNull() || payload.isMissingNode() || payload.isEmpty()
                ? fail(
                        "empty payload",
                        "non-empty financial statement",
                        "Financial statement payload is empty")
    ...
    private Outcome title(JsonNode payload) {
        ...
        if (items.isEmpty())
            return fail(
                    ...
                    "News payload has no item to validate");
    ...
    private Outcome url(JsonNode payload) {
        ...
        if (items.isEmpty())
            return fail(
                    "no news item",
                    "at least one linked news item",
                    "News payload has no item to validate");
        for (JsonNode item : items) {
            String value = text(item, "url", "link", "href");
            if (!isHttpUrl(value))
                return fail(
                        value.isBlank() ? "missing link" : value,
                        "valid http(s) URL",
                        "A news item has no valid source link");
        }
    ...
    private Outcome itemCode(JsonNode payload) {
        ...
        if (!data.isArray())
            return fail(
                    ...
                    "array of statement items",
                    "Financial statement data is malformed");
        for (JsonNode item : data)
            if (item.path("itemCode").asText().isBlank())
                return fail(
                        "missing itemCode",
                        "non-blank itemCode",
                        "Financial statement item has no code");
    ...
    private Outcome envelope(JsonNode payload) {
        if (!hasDataEnvelope(payload)
                || payload.path("provider").asText().isBlank()
                || payload.path("dataset").asText().isBlank()
                || payload.path("retrieved_at").asText().isBlank())
            return fail(
                    "incomplete envelope",
                    "provider, dataset, retrieved_at and data",
                    "Data-bearing raw response envelope is incomplete");
    ...
    private Outcome dataCount(JsonNode payload) {
        ...
        return payload.path("count").asInt(-1) == payload.path("data").size()
                ...
                : fail(
                        "count=" + payload.path("count").asText(),
                        "data.size=" + payload.path("data").size(),
                        "Envelope count does not match data array");
    }
    ...
    private Outcome duplicateNews(RawPayloadEntity raw) {
        return rawPayloads.existsByDataSourceIdAndChecksumSha256AndIdNot(
                ...
                ? fail(
                        raw.getChecksumSha256(),
                        "unique checksum per source",
                        "Duplicate news raw payload; no new data version will be created")
    ...
    private Outcome errorMessage(JsonNode payload, String rawText) {
            ...
            for (String field : List.of("error", "errors", "failed"))
                if (payload.hasNonNull(field))
                    return fail(
                            ...
                            "Payload contains an upstream error marker");
        ...
        return lower.contains("\"error\"")
                ...
                ? fail(
                        ...
                        "successful data payload",
                        "Payload contains an upstream error message")
    ...
    private String text(JsonNode item, String... keys) {
        for (String key : keys)
            if (item.path(key).isTextual() && !item.path(key).asText().isBlank())
                return item.path(key).asText().trim();
        return "";
    }
