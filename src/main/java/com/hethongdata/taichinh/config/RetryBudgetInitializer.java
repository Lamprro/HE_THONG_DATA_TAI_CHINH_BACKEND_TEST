...
@Component
@ConditionalOnProperty(prefix = "financial.ingestion.retry-budget", name = "initialize-enabled", havingValue = "true")
public class RetryBudgetInitializer implements ApplicationRunner {
    ...
    public RetryBudgetInitializer(IngestionJobRepository ingestionJobs) { this.ingestionJobs = ingestionJobs; }
    @Override public void run(ApplicationArguments args) {
        LOGGER.info("Initialized retry budget 10 for {} ingestion jobs without changing activation state",
