    ...
    public IngestionExecutionResponse execute(IngestionJobEntity job, String triggerType) {
        ...
        for (DataVersionEntity version : candidates) {
            ...
            try {
                ...
                throw new IngestionExecutionException(run.getId(), ExternalErrorCategory.PROTOCOL,
                        null, workflow + " failed; xem log run " + run.getId(), exception);
            }
        }
        return response(lastRunId);
    }

    private IngestionExecutionResponse response(UUID runId) {
        return new IngestionExecutionResponse(runId, null, "SUCCESS", 200,
