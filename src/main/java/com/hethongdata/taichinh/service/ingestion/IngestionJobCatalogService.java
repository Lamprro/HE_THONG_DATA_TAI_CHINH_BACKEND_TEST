...
public class IngestionJobCatalogService {
    private static final List<String> RETIRED_PAID_NEWS_JOB_CODES =
            List.of(
                    "VNSTOCK_NEWS_STATUS_DAILY",
                    "VNSTOCK_NEWS_SITES_DAILY",
                    "VNSTOCK_NEWS_FPT_30M");
    ...
    private void seedSources() {
        ...
        // VNSTOCK_NEWS source remains in data_sources because existing run/raw history references
        // it.
    }
    ...
    private void equityJobs(
        ...
        jobs.add(
                job(
                        prefix + "_OHLCV_DAILY",
                        source + " FPT OHLCV trailing window",
                        source,
                        "MARKET_PRICE",
        ...
        for (String statement : List.of("balance_sheet", "income_statement", "cash_flow")) {
            ...
            jobs.add(
                    job(
                            prefix + "_" + statement.toUpperCase() + "_WEEKLY",
                            source + " FPT " + statement,
                            source,
                            "FINANCIAL_STATEMENT",
                            WEEKLY_UTC,
                            "FINANCIAL_STATEMENT",
    ...
    private void cafeFJobs(List<JobDefinition> jobs) {
        ...
        for (String operation : List.of("MANAGEMENT", "SUBSIDIARIES")) {
            jobs.add(
                    job(
                            "CAFEF_FPT_" + operation + "_WEEKLY",
                            "CafeF FPT " + operation.toLowerCase(),
                            "CAFEF",
                            "COMPANY",
                            WEEKLY_UTC,
                            operation,
